          ...
          <Section icon={Calendar} title={t.concertReviews} tone="amber">
            <div className="grid md:grid-cols-2 gap-5">
              {concertReviews.map((r) => {
                const shownQuote = r.quote_text_i18n?.[lang] || r.quote_text;
                const isTranslated = r.quote_language && r.quote_language !== lang && !!r.quote_text_i18n?.[lang];
                return (
                <figure key={r.id} className="border border-amber-400/20 rounded-lg p-5 bg-amber-400/5">
                  <blockquote className="text-foreground/90 italic leading-relaxed mb-3">
                    "{shownQuote}"
                    {isTranslated && (
                      <span className="ml-2 text-[10px] uppercase text-amber-300/70 not-italic">({t.translatedFrom} {r.quote_language})</span>
                    )}
                  </blockquote>
                  <figcaption className="text-sm text-muted-foreground">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-[10px] tracking-[0.2em] uppercase text-muted-foreground border-b border-border">
                      <th className="p-3">{t.year}</th><th className="p-3">{t.title}</th><th className="p-3">{t.producer}</th><th className="p-3">{t.studio}</th><th className="p-3">{t.label}</th><th className="p-3">{t.chart}</th><th className="p-3">{t.sales}</th><th className="p-3">{t.musicians}</th><th className="p-3">YouTube</th><th className="p-3">{t.notes}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {discography.map((d, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-border/50 last:border-0 align-top">
                          ...
                          <td className="p-3 text-muted-foreground">{d.studio ?? "—"}</td>
